...
public:
    ...
    bool isvalid(int n, int val, vector<int>& quantities) {
        ...
        for (auto it : quantities) {
            // current item has 'it' no of products, har shop ko val jitne
            // products do
            if (it % val == 0) {
                // means har shop ko val jitne products milenge
                // eg if val=5 and it=15, products split to shop like {5,5,5},
                // ie each shop gets 5 products and total shops filled=it/val=3
                shopsfilled += it / val;
            } else {
                ...
                // eg if val=5 and it=12, products split to shop like {5,5,5,2},
                // ie each shop gets 5 products(except last shop) and total
                // shops filled=(it/val)+1=3+1=4
                shopsfilled += (it / val) + 1;
            }

            // if at any point shops filled>n, means zyada shops bhar di hai,
            // items will be left undistributed
        ...
        // since no product left hence we can fill all shops with each shop
    ...
    int minimizedMaximum(int n, vector<int>& quantities) {
        ...
        while (left <= rt) {
            ...
            // it at max 5 product per shop

            // like in eg1, if mid=5,
            // distribute 11 like 5,5,1(since ek store ko ek product type hi de
            // sakte hai) , ttl stores done=3
            // distribute next item-> 6 like 5,1(since ek store ko ek product
            // type hi de sakte hai) ttl stores done=2 overall stores
            // filled=3+2=5, stores filled like {5,5,1,5,1,0}(0 filled wala bhi
            // valid hai accd to question)
            // now all stores are filled no problem,
            // and no product remaining out, so this mid is a possible ans,
            // since possible hai mid jitne parts karke fill karna in such case
            // maxm value of product recieved by any store=5, ab isko minimize
            // karna hai toh lets try with mid=2

            // mid=2 distribute 11 like
            // 2,2,2,2,2,1(since ek store ko ek product type hi de sakte hai ) ,
            // ttl stores done=6 stores filled like {2,2,2,2,2,1} but abhi toh
            // item ->6 bacha hai for distribution, and already stores have
            // been filled, that means all product not distributed, this is not
            // a possible ans for our question, hence increase mid value and try
            // again, yehi karna hai basically
