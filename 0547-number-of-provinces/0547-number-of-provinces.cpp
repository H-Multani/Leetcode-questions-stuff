...
public:
    int findCircleNum(vector<vector<int>>& isConnected) {
        ...
        // declared dsu, initially none of the nodes are connected to each other
        DSU dsu(nodes);

        // connect the nodes as said by the questn
        for (auto i = 0; i < isConnected.size(); i++) {
            for (auto j = 0; j < isConnected[0].size(); j++) {
                // if isConnected[i][j]==1 means i and j th node ke beech me
                // edge hai, connect karo dono(i,j th node) ko
