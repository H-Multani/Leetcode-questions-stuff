...
public:
    vector<int> processQueries(int c, vector<vector<int>>& connections,
        ...
        // element hai like if towers 2,4,5,1 are on grid 2, then map me key
        ...
        // update kardo sabke super parents nai toh ho sakta hai galat parent ke
        // set me daal de tower ko
        grid.update();
        ...
        for (auto it : queries) {
            ...
            if (it[0] == 2) {
                ...
                if (towers[parentt].find(x) == towers[parentt].end()) {
                    // means nai hai, in such case smallest id nikal lo same
                    // grid se
                    ...
                    // set hai toh smallest id at beginning hogi 
                    int id = *towers[parentt].begin();

                    // iss wali id ko ans me daal do
                    ans.push_back(id);
                } else {
                    // we here means ki tower is available, in such case khud hi
                    // manage kar lega tower
                    ans.push_back(x);
                }
            }
        }
